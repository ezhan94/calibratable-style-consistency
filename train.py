...
from util.environments import load_environment
...
def run_epoch(data_loader, model, device, env, train=True, early_break=False):
    ...
    for batch_idx, (states, actions, labels_dict) in enumerate(data_loader):
        ...
        if model.requires_labels:
            ...
            if model.requires_environment:
                batch_log = model(states, actions, env)
            else:
                batch_log = model(states, actions)
...
def start_training(save_path, data_config, model_config, train_config, device, test_code=False):
    ...
    env = None
    if model_class.requires_environment:
    ...
    for num_epochs in train_config['num_epochs']:
        ...
        if model.stage < len(train_config['num_epochs']):
            ...
            model.filter_and_load_state_dict(best_state)
    ...
    # TODO hacky solution, since label functions are not currently JSON serializable
    if model_class.requires_labels:
